...
RANGE_FIELDS = [
    ...
    ("diameter_sd_nm", "low"),
    ("zeta_abs_mV", "high"),
    ("diameter_DLS_nm", "range"),
    ("diameter_TEM_nm", "range"),
    ("emission_peak_nm", "range"),
    ("synthesis_temperature_C", "range"),
    ("synthesis_time_h", "range"),
    ("pH", "range")
]
...
STRUCTURE_FIELDS = [
    ...
    "pdi_or_cv", "zeta_mV", "emission_peak_nm",
    "synthesis_temperature_C", "synthesis_time_h", "pH",
    "AuNC_type", "is_pure_AuNC_or_composite", "composite_type",
    "carrier_or_matrix", "reference_id", "DOI", "paper_title"
]

def add_zeta_abs(rows):
    for row in rows:
        z = num(row.get("zeta_mV"))
        row["zeta_abs_mV"] = abs(z) if not math.isnan(z) else ""
    return rows

def parameter_ranges(rows):
    rows = add_zeta_abs([dict(r) for r in rows])
    result = []
    for field, mode in RANGE_FIELDS:
        vals = [num(r.get(field)) for r in rows]
        vals = [v for v in vals if not math.isnan(v)]
        if not vals:
            continue
        ...
        q75 = percentile(vals, 0.75)
        if mode == "low":
            rec_low, rec_high = min(vals), med
        elif mode == "high":
            rec_low, rec_high = med, max(vals)
        else:
            rec_low, rec_high = q25, q75
        result.append({
            "parameter": field,
            "n": len(vals),
            "min": min(vals),
...
def range_score(value, low, high, rule):
    if math.isnan(value) or math.isnan(low) or math.isnan(high):
        return math.nan
    if low > high:
        low, high = high, low
    if rule == "low":
        return 1.0 if value <= high else max(0.0, 1.0 - (value - high) / (abs(high) + 1e-9))
    if rule == "high":
        return 1.0 if value >= low else max(0.0, 1.0 - (low - value) / (abs(low) + 1e-9))
    if low <= value <= high:
        return 1.0
    if value < low:
        return max(0.0, 1.0 - (low - value) / (abs(low) + 1e-9))
    return max(0.0, 1.0 - (value - high) / (abs(high) + 1e-9))

def score_rows(rows, ranges):
    lookup = {r["parameter"]: r for r in ranges}
    output = []
    for row in add_zeta_abs([dict(r) for r in rows]):
        item = dict(row)
        scores = []
        for field, rule in RANGE_FIELDS:
            if field not in lookup:
                continue
            info = lookup[field]
            value = num(item.get(field))
            score = range_score(value, num(info["recommended_low"]), num(info["recommended_high"]), info["rule"])
            item[f"{field}_score"] = score if not math.isnan(score) else ""
            if not math.isnan(score):
                scores.append(score)
        item["dispersity_score"] = sum(scores) / len(scores) if scores else ""
        output.append(item)
    output.sort(key=lambda r: num(r.get("dispersity_score")), reverse=True)
    for i, row in enumerate(output, 1):
        row["dispersity_rank"] = i
    return output

def literature_recommendations(rows):
    useful = []
    for row in rows:
        support = 0
        for field, rule in RANGE_FIELDS:
            if field == "zeta_abs_mV":
                if not math.isnan(num(row.get("zeta_mV"))):
                    support += 1
            elif not math.isnan(num(row.get(field))):
                support += 1
        if support:
            item = {k: row.get(k, "") for k in STRUCTURE_FIELDS}
            item["supporting_parameter_count"] = support
            useful.append(item)
    useful.sort(key=lambda r: int(r["supporting_parameter_count"]), reverse=True)
    for i, row in enumerate(useful, 1):
        row["rank"] = i
    return useful

def main():
    rows = [r for r in read_csv(DATA_FILE) if r.get("analysis_group") == "stability"]
    candidates = read_csv(CANDIDATE_FILE)
    ranges = parameter_ranges(rows)
    lit = literature_recommendations(rows)
    write_csv(RESULTS / "high_dispersity_parameter_ranges.csv", ranges)
    write_csv(RESULTS / "high_dispersity_structure_recommendations.csv", lit[:30])

    if candidates:
        scored = score_rows(candidates, ranges)
        write_csv(RESULTS / "high_dispersity_candidate_scores.csv", scored)
        write_csv(RESULTS / "top_high_dispersity_candidates.csv", scored[:10])
    else:
        write_csv(RESULTS / "high_dispersity_candidate_scores.csv", [], ["candidate_id", "message"])
        write_csv(RESULTS / "top_high_dispersity_candidates.csv", [], ["candidate_id", "message"])

    print("high dispersity structure prediction finished")
